...
class PaginationGeneratorTest {
    ...
    void testGeneratePaginationHelper() {
        ...
        assertTrue(output.contains("ListItemsInput -> '{IO, AWSEnv, Http, Exception, Threads} [Item]"),
            "Should have correct signature with concrete item type and AWSEnv/Http abilities");
        
        // Verify recursive structure - now uses concrete type [Item]
        assertTrue(output.contains("go : Optional Text -> [Item] -> '{IO, AWSEnv, Http, Exception, Threads} [Item]"),
            "Should have recursive go helper with concrete type and AWSEnv/Http abilities");
